...
const listenToSearch = () => {
    ...
    addFilteredMovies(movies, event.target.value)
	)
};
  ...
  // Adds all movies
  const addMoviesToDom = function addMoviesToDom() {
  ...
  //Adds newest movies
  const addNewestMoviesToDom = function addNewestMoviesToDom() {
  ...
  //Adds filtered movies Avengers, X-men, Princess & Batman
  ...
  const addEventListeners = function addEventListeners() {
    ...
    all_films.addEventListener("change", function() {
        console.log("all films selected");
        removeAllItems()
        addMoviesToDom();
      });
